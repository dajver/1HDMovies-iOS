...
class MoviesRepository {
    ...
    func fetchMovies(page: Int) async throws -> [MoviesDataModel] {
        let html = try await HttpClient.shared.get("\(Config.baseURL)/movies/page/\(page)")
        ...
        for index in 0..<thumbnails.count {
            ...
            movies.append(MoviesDataModel(name: name, thumbnail: thumbnail, link: link, type: .movie, quality: quality, other: releaseYear))
        }
        return movies
    }
}
